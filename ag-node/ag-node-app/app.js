...
function testText() {
    ...
    var sentences = [undefined, '', {}, 123, -123, 'a', 'a good day', 'aGoodDay', 'a-good-day', 'a_good_day', 'a    good    day', 'A GOOD DAY',    ['a good case','bGoodCase']];
...
function testNumber() {
    ...
    var numbers = [undefined, '', {}, 0, 123, 123.45, 12345677.45677, 'abc', [[1, -1], 1231232, -12123]];
...
function testMoney() {
    var monies = [ undefined, {}, {m:0, c:'USD'}, {m:1234567.8903, c:'EUR'},[{m:-1, c:'USD'}, {m:7685437.8903, c:'EUR'}]];
...
function testAcct() {
    var accts = [undefined, '', {}, 123455, '12123', '123456789', '1234567890', ['12','1212']];
...
function testAccounting() {
    ...
    console.log('a: ' + accounting.formatMoney(12344.50));
};
...
function testFs() {
    ...
    var dir = process.env.TECH_HOME; // read from the env variables

    fs.readdir(dir, function(err, files) {
        var size = _.size(files);
        console.log('        size: ' + size);

        var foundBin = _.find(files, function(f) { return f == 'bin'; });
        console.log('        looking for bin: ' + foundBin);

        files = _.filter(files, function(x) { return x.charAt(0) != '.'; }); // skip hidden files
        console.log('        listing files ...');
        _.each(files, function(x) { console.log('            :' + x); });
